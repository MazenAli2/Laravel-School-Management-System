                    ...
                    <form method="POST" action="{{ route('admin.timetables.update', $timetable) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="section_id" id="section_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                                ...
                                @foreach($sections as $section)
                                    <option value="{{ $section->id }}" {{ $timetable->section_id == $section->id ? 'selected' : '' }}>{{ $section->grade->name }} - {{ $section->name }}</option>
                                @endforeach
                            </select>
                            @error('section_id')
                        ...
                        <div class="mb-4">
                            ...
                            <select name="teacher_id" id="teacher_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                                ...
                                @foreach($teachers as $teacher)
                                    <option value="{{ $teacher->id }}" {{ $timetable->teacher_id == $teacher->id ? 'selected' : '' }}>{{ $teacher->user->name }}</option>
                                @endforeach
                            </select>
                            @error('teacher_id')
                                <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <input type="time" name="start_time" id="start_time" value="{{ \Carbon\Carbon::parse($timetable->start_time)->format('H:i') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                                @error('start_time')
                                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="time" name="end_time" id="end_time" value="{{ \Carbon\Carbon::parse($timetable->end_time)->format('H:i') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                                @error('end_time')
                                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                @enderror
                            </div>
